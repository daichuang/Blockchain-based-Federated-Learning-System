...
import torch
dtype_int = torch.cuda.IntTensor if torch.cuda.is_available() else torch.IntTensor
...
class EncodedNumber(object):
    ...
    Attributes:
      ...
      exponent (int): Together with :attr:`BASE`, determines the level
        of fixed-precision used in encoding the number.
    """
    BASE =  torch.as_tensor(16).type(dtype_int)
    BASE_float =  torch.as_tensor(16.0).type(dtype)
    ...
    """
    LOG2_BASE =  torch.as_tensor(4.0).type(dtype_int)
    FLOAT_MANTISSA_BITS =  torch.as_tensor(sys.float_info.mant_dig).type(dtype_int)

    def __init__(self, public_key, encoding, exponent):
        self.public_key = public_key
        self.encoding = encoding
        self.exponent = torch.as_tensor(exponent).type(dtype_int)
    ...
    def encode(cls, public_key, scalar, precision=None, max_exponent=None):
        ...
        *max_exponent*, and such that the number *precision* is not
        ...
        if precision is None:
            #print((scalar.dtype))
            if  (scalar.dtype ==  torch.int64) or(scalar.dtype ==  torch.int32)  :
                prec_exponent = 0
            elif  (scalar.dtype==torch.float32) or scalar.dtype==torch.float64:
                # Encode with *at least* as much precision as the python float
                # What's the base-2 exponent on the float?
                bin_flt_exponent = torch.frexp(scalar)[1]
                ...
                prec_exponent = torch.floor(bin_lsb_exponent / cls.LOG2_BASE)
            else:
                raise TypeError("Don't know the precision of type %s."
                                % type(scalar))
        else:
            prec_exponent = torch.floor(torch.log(precision, cls.BASE_float))
        ...
        if max_exponent is None:
            ...
            exponent = torch.min(max_exponent, prec_exponent)
            
        
        #print(exponent)
        #print('-------------')
        #sys.exit(0)
        exponent=torch.as_tensor(exponent).type(dtype)
        temp=torch.pow(cls.BASE_float, -exponent)
        
        temp=torch.as_tensor(temp).type(dtype)
 
        temp=  scalar *  temp 
        #print(temp)
        if  not  temp.dtype ==  torch.int32  and not  temp.dtype ==  torch.int64 :
    ...
    def decode(self):
        ...
        #print( self.exponent)
        #print(  )
        return mantissa *  pow(float(self.BASE), float(self.exponent))
