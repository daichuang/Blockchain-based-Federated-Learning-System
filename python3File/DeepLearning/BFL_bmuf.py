...
def trimed_Bmuf( flaten_Global_weights , flaten_grads , client ) :
    global smoothed_grads
    #######################(0) grads -- after calc_grads on local client --> turn to list(tensor)
    Global_weights = client.model.reshape_flatten_arg(np.array(flaten_Global_weights))  # [tensor]
    ...
    grads = client.model.reshape_flatten_arg(flaten_grads_after_byzantine.numpy())  # [tensor]
